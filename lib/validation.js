      ...
      for (let i = 0; i < args.length; i++) {
        ...
        if (inferredType.type && inferredType.confidence === 'high') {
          if (!isTypeCompatible(inferredType.type, expectedParam.type)) {
            // Build enhanced type message with rmType info and conversion suggestions
            let typeMessage = `Argument ${i + 1} (${expectedParam.name}): expected ${formatTypeName(expectedParam.type)}, but got ${formatTypeName(inferredType.type)}`;
            
            // Try to narrow the range to just the problematic part (e.g., the variable)
            let diagnosticRange = argRange;
            const trimmedArg = arg.text.trim();
            
            // Check if the issue involves a variable with an incompatible rmType
            let varInfo = null;
            let varName = null;
            const hasOperators = /[+\-*\/%&<>=!]/.test(trimmedArg);
            
            // Extract variable info to provide better diagnostics
            if (trimmedArg.startsWith('$')) {
              const varMatch = trimmedArg.match(/^\$([a-zA-Z_][a-zA-Z0-9_]*)/);
              if (varMatch) {
                varName = varMatch[1];
                varInfo = variables?.get(varName);
              }
            }
            
            // Enhance message with rmType and conversion suggestion
            if (varInfo?.rmType) {
              typeMessage += `. Variable $${varName} is ${varInfo.rmType}`;
              
              // Suggest VALUE() for numeric conversions from text types
              if (['number', 'integer'].includes(expectedParam.type) && 
                  ['text', 'iso8601_datetime', 'iso8601_date', 'iso8601_time'].includes(inferredType.type)) {
                typeMessage += `. Use VALUE($${varName}) to convert to number`;
              }
            }
            
            // Only narrow to just the variable if the expression is ONLY a variable reference
            // If there are operators/comparisons, keep the full expression highlighted
            if (!hasOperators && trimmedArg.startsWith('$')) {
              // Match variable with path accessors (e.g., $var/path/to/field)
              const varMatch = trimmedArg.match(/^\$[a-zA-Z_][a-zA-Z0-9_]*(\/[a-zA-Z_][a-zA-Z0-9_]*)*/);
              if (varMatch && varMatch[0] === trimmedArg) {
                // Only narrow if the match is the entire argument (not just a prefix)
                const varText = varMatch[0];
                const varStartInArg = arg.text.indexOf(varText);
                const varStartInDoc = argStartInDoc + varStartInArg;
                const varEndInDoc = varStartInDoc + varText.length;
                const varStartPos = document.positionAt(varStartInDoc);
                const varEndPos = document.positionAt(varEndInDoc);
                diagnosticRange = new vscode.Range(varStartPos, varEndPos);
              }
            }
...
/**
 * Detect variables used without the $ prefix.
 * @param {object} vscode - VS Code API
 * @param {object} document - TextDocument
 * @param {Map<string, VariableInfo>} variables - Variables from form_description.json
 * @param {Map<string, any>} functionLookup - Function metadata lookup
 * @returns {Array} - Array of diagnostics
 */
function detectMissingDollarSign(vscode, document, variables, functionLookup) {
  ...
  // Build a list of string ranges to exclude from checking
  const stringRanges = [];
  ...
  // Match identifiers that could be variable names (not starting with $)
  ...
  while ((match = identifierRegex.exec(text)) !== null) {
    ...
    // Skip if preceded by $ (already has the dollar sign)
    ...
    if (functionLookup && functionLookup.has(identifierUpper)) {
      continue;
    }
    
    // Skip boolean literals and null
    if (/^(true|false|null)$/i.test(identifier)) {
      continue;
    }
    
    // Check if this identifier matches a known variable name
    if (variables.has(identifier)) {
      ...
      const diagnostic = new vscode.Diagnostic(
        range,
        `Variable '${identifier}' must be prefixed with $. Use $${identifier} instead`,
        vscode.DiagnosticSeverity.Error
      );
      diagnostic.source = 'dips-calc';
      diagnostic.code = 'missing-dollar-sign';
      diagnostics.push(diagnostic);
    }
  }
...
/**
 * Detect variables used that are not declared in form_description.json.
 * @param {object} vscode - VS Code API
 * @param {object} document - TextDocument
 * @param {Map<string, VariableInfo>} variables - Variables from form_description.json
 * @returns {Array} - Array of diagnostics
 */
function detectUndeclaredVariables(vscode, document, variables) {
  if (!variables || variables.size === 0) {
    // If no form_description.json is available, don't warn
    return [];
  }
  ...
  // Match variable references (starting with $)
  ...
  while ((match = variableRegex.exec(text)) !== null) {
    ...
    // Check if this variable is NOT in the form_description.json
    if (!variables.has(varName)) {
      ...
      const diagnostic = new vscode.Diagnostic(
        range,
        `Variable '$${varName}' is not declared in form_description.json`,
        vscode.DiagnosticSeverity.Warning
      );
      diagnostic.source = 'dips-calc';
