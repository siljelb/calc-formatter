            ...
            if (pathMatch) {
              ...
              if (varInfo) {
                ...
                if (varInfo.values && varInfo.values.length > 0) {
                  // For DV_QUANTITY, show units separately (not as "allowed values")
                  if (varInfo.rmType === 'DV_QUANTITY') {
                    markdown.appendMarkdown('**Allowed units:**\n\n');
                    const maxDisplay = 10;
                    const displayValues = varInfo.values.slice(0, maxDisplay);
                    for (const val of displayValues) {
                      let unitDisplay = `- \`${val.value}\``;
                      
                      // Add range information if available
                      if (val.range) {
                        const rangeParts = [];
                        if (val.range.min !== undefined) {
                          const minOp = val.range.minOp || '>=';
                          rangeParts.push(`${minOp} ${val.range.min}`);
                        }
                        if (val.range.max !== undefined) {
                          const maxOp = val.range.maxOp || '<=';
                          rangeParts.push(`${maxOp} ${val.range.max}`);
                        }
                        if (rangeParts.length > 0) {
                          unitDisplay += ` (${rangeParts.join(', ')})`;
                        }
                      }
                      
                      if (val.label !== val.value) {
                        unitDisplay += ` — ${val.label}`;
                      }
                      markdown.appendMarkdown(unitDisplay + '\n');
                    }
                    if (varInfo.values.length > maxDisplay) {
                      markdown.appendMarkdown(`\n*...and ${varInfo.values.length - maxDisplay} more*\n`);
                    }
                  } else {
                    // For other types, show as allowed values
