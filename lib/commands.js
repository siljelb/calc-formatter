 ...
 * - expandCustomFunctions - Expands custom function macros
 */

const { expandCustomFunctions: expandCustom } = require('./custom-functions/index');

/**
 * Helper to transform document or selection.
 * @param {object} vscode - The VS Code API
 * @param {vscode.TextEditor} editor - The active editor
 * @param {Function} transformFn - The transformation function
 */
async function transformDocumentOrSelection(vscode, editor, transformFn) {
...
function createCommands(vscode, formatFormula, minifyFormula, stripComments) {
  ...
  const minifyCommand = vscode.commands.registerCommand('dips-calc.minifyFormula', async () => {
    ...
    await transformDocumentOrSelection(vscode, editor, minifyFormula);
  });
  ...
  const beautifyCommand = vscode.commands.registerCommand('dips-calc.beautifyFormula', async () => {
    ...
    await transformDocumentOrSelection(vscode, editor, formatFormula);
  });
  ...
  const stripCommentsCommand = vscode.commands.registerCommand('dips-calc.stripComments', async () => {
    ...
    await transformDocumentOrSelection(vscode, editor, stripComments);
    vscode.window.showInformationMessage('Comments stripped. The result is ready for use in DIPS Arena.');
  });

  // Expand custom functions command
  const expandCustomFunctionsCommand = vscode.commands.registerCommand('dips-calc.expandCustomFunctions', async () => {
    const editor = vscode.window.activeTextEditor;
    if (!editor || editor.document.languageId !== 'dips-calc') {
      vscode.window.showInformationMessage('Open a DIPS Calc expression file before expanding custom functions.');
      return;
    }
    await transformDocumentOrSelection(vscode, editor, expandCustom);
    vscode.window.showInformationMessage('Custom functions expanded.');
  });

  // Export for Arena command (expand + strip)
  const exportForArenaCommand = vscode.commands.registerCommand('dips-calc.exportForArena', async () => {
    const editor = vscode.window.activeTextEditor;
    if (!editor || editor.document.languageId !== 'dips-calc') {
      vscode.window.showInformationMessage('Open a DIPS Calc expression file before exporting.');
      return;
    }

    // Chain: expand custom functions -> strip comments
    const expanded = expandCustom(editor.document.getText());
    const noComments = stripComments(expanded);
    ...
    vscode.window.showInformationMessage('Formula exported for DIPS Arena (custom functions expanded, comments stripped).');
  });
  ...
  return [
    ...
    expandCustomFunctionsCommand,
    exportForArenaCommand
  ];
}
